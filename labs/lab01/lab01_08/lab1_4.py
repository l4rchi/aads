...
n_values = list(range(1, max_n, 800))
average_times = []


@timing_decorator(ndigits, number_of_runs)
def calculate_average_harmonic(vector):
    return statistics.harmonic_mean(vector)


for n in n_values:
    print(n)

    average_time = calculate_average_harmonic(max_vector[:n])
...
plt.savefig('average_harmonic.png')